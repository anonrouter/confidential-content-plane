...
const envSchema = z.object({
  ...
  // Media (image/TTS) endpoints. When disabled they return an explicit 503
  // rather than a silent 404. The split control role defaults them off (media
  // needs the Venice credential, which control does not hold). This governs the
  // legacy monolith speech route; split image generation has its own flag below.
  MEDIA_ENABLED: z.enum(["true", "false"]).default("true").transform((value) => value === "true"),
  // Split-architecture image generation. Default OFF everywhere and enforced
  // server-side and independently at the relay ingress/route and the control
  // authorization. Unlike MEDIA_ENABLED this makes no monolith assumption: the
  // prompt/image never touch control, the worker holds the Venice credential,
  // and control only authorizes and settles the flat price. Enable per role in a
  // staged rollout (see docs/SPLIT_MEDIA_IMAGE.md). Missing/false fails closed
  // with the same clear 503 media_disabled.
...
export function loadConfig() {
  ...
  if (env.CONNECT_ENABLED) {
    if (env.RUNTIME_ROLE !== "api" && env.RUNTIME_ROLE !== "control") {
      throw new Error("AnonRouter Connect can only run on the api or control role");
    }
  ...
  return {
    ...
    internal: {
      ...
      // Control never holds the Venice key, so the legacy monolith media (speech)
      // route is disabled there by default.
      mediaEnabled: env.RUNTIME_ROLE === "control" ? false : env.MEDIA_ENABLED,
