export { getComposeHash } from './get-compose-hash.mjs';
export { VerifyOptions, verifyEnvEncryptPublicKey, verifyEnvEncryptPublicKeyLegacy } from './verify-env-encrypt-public-key.mjs';

interface GetTlsKeyResponse {
...
type TcbInfoV03x = TcbInfo & {
    rootfs_hash?: string;
};
type TcbInfoV05x = TcbInfo & {
    mr_aggregated: string;
    os_image_hash: string;
    compose_hash: string;
    device_id: string;
};
interface InfoResponse<VersionTcbInfo extends TcbInfo> {
    app_id: string;
    instance_id: string;
    app_cert: string;
...
declare class DstackClient<T extends TcbInfo = TcbInfoV05x> {
    protected endpoint: string;
    constructor(endpoint?: string | undefined);
    private ensureAlgorithmSupported;
    private ensureTlsKeyOptionsSupported;
    getKey(path?: string, purpose?: string, algorithm?: string): Promise<GetKeyResponse>;
    ...
    attest(report_data: string | Buffer | Uint8Array): Promise<AttestResponse>;
     ...
     */
    sign(algorithm: string, data: string | Buffer | Uint8Array): Promise<SignResponse>;
    /**
     ...
     */
    verify(algorithm: string, data: string | Buffer | Uint8Array, signature: string | Buffer | Uint8Array, publicKey: string | Buffer | Uint8Array): Promise<VerifyResponse>;
    /**
...
declare class TappdClient extends DstackClient<TcbInfoV03x> {
    constructor(endpoint?: string | undefined);
    /**
     * @deprecated Use getKey instead.
     * @param path The path to the key.
